...
function sphereBox(sphere, box) {
  ...
  const delta = closest.clone().sub(sphere.position);
...
function collide(a, b) {
  ...
  if (a.shape.type === 'box' && b.shape.type === 'sphere') { const c = sphereBox(b, a); if (!c) return null; c.normal.multiplyScalar(-1); return c; }
