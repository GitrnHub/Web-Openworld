...
function makeSurfacePack(baseHex, seed, opts = {}) {
  ...
  // THREE.Color stores hex/CSS colors in Linear-sRGB. Canvas color textures are tagged sRGB,
  // so encode the procedural texels back to sRGB before upload to avoid double-linearizing them.
...
export const DAMAGE_PROFILES = Object.freeze({
  glass: {
    label: '玻璃', response: 'brittle', fractureThreshold: 1.0, integrity: 4.8,
    dentScale: 0.015, recovery: 0, damageRadius: 0.95, fractureRadius: 0.72,
    fragmentCount: [16, 30], fragmentScale: [0.05, 0.34], fragmentShape: 'glass',
    restitution: 0.24, friction: 0.76, dust: 8, crackLines: 9, color: 0xbfeaff,
  },
  ceramic: {
    label: '陶瓷', response: 'brittle', fractureThreshold: 1.65, integrity: 6.8,
    dentScale: 0.025, recovery: 0, damageRadius: 0.8, fractureRadius: 0.56,
    fragmentCount: [10, 22], fragmentScale: [0.07, 0.42], fragmentShape: 'ceramic',
    restitution: 0.15, friction: 0.82, dust: 15, crackLines: 7, color: 0xe8e4d8,
  },
  plaster: {
    label: '石膏板', response: 'crumbly', fractureThreshold: 1.8, integrity: 8.0,
    dentScale: 0.13, recovery: 0, damageRadius: 0.9, fractureRadius: 0.6,
    fragmentCount: [8, 18], fragmentScale: [0.08, 0.34], fragmentShape: 'chunk',
    restitution: 0.08, friction: 0.88, dust: 28, crackLines: 4, color: 0xd4d0c6,
  },
  concrete: {
    label: '混凝土', response: 'crumbly', fractureThreshold: 3.1, integrity: 14.0,
    dentScale: 0.075, recovery: 0, damageRadius: 0.82, fractureRadius: 0.48,
    fragmentCount: [7, 15], fragmentScale: [0.09, 0.4], fragmentShape: 'chunk',
    restitution: 0.07, friction: 0.92, dust: 38, crackLines: 3, color: 0x8d918f,
  },
  wood: {
    label: '木材', response: 'fibrous', fractureThreshold: 2.35, integrity: 10.0,
    dentScale: 0.1, recovery: 0.03, damageRadius: 0.8, fractureRadius: 0.5,
    fragmentCount: [8, 16], fragmentScale: [0.06, 0.48], fragmentShape: 'splinter',
    restitution: 0.12, friction: 0.84, dust: 12, crackLines: 4, color: 0xa86f3d,
  },
  metal: {
    label: '金属', response: 'ductile', fractureThreshold: 8.5, integrity: 24.0,
    dentScale: 0.28, recovery: 0.005, damageRadius: 0.9, fractureRadius: 0.42,
    fragmentCount: [2, 7], fragmentScale: [0.08, 0.38], fragmentShape: 'flake',
    restitution: 0.3, friction: 0.68, dust: 1, crackLines: 0, color: 0x7f8a95,
  },
  plastic: {
    label: '塑料', response: 'ductile', fractureThreshold: 5.5, integrity: 16.0,
    dentScale: 0.36, recovery: 0.018, damageRadius: 1.0, fractureRadius: 0.5,
    fragmentCount: [4, 10], fragmentScale: [0.07, 0.38], fragmentShape: 'flake',
    restitution: 0.22, friction: 0.72, dust: 2, crackLines: 1, color: 0x2b82a8,
  },
  rubber: {
    label: '弹性体', response: 'elastic', fractureThreshold: 12.0, integrity: 32.0,
    dentScale: 0.55, recovery: 2.7, damageRadius: 1.1, fractureRadius: 0.35,
    fragmentCount: [1, 3], fragmentScale: [0.12, 0.3], fragmentShape: 'chunk',
    restitution: 0.68, friction: 0.8, dust: 0, crackLines: 0, color: 0x252a31,
  },
});
...
export function createMaterials(renderer) {
  ...
  const tagMapped = (material, metersPerTile) => {
    material.userData.worldUvMetersPerTile = metersPerTile;
    material.userData.proceduralPBR = true;
    material.userData.tileableTextures = true;
    return material;
  };
  ...
  return {
    ...
    glass: physical({
      color: 0xd7f2ff, roughness: 0.055, metalness: 0, transmission: 0.93, thickness: 0.10, ior: 1.48,
      transparent: false, opacity: 1, side: THREE.DoubleSide, clearcoat: 0.18, clearcoatRoughness: 0.10, envMapIntensity: 1.2,
    }),
    glassDark: physical({
      color: 0x8fb3c1, roughness: 0.09, metalness: 0, transmission: 0.78, thickness: 0.14, ior: 1.48,
      transparent: false, opacity: 1, side: THREE.DoubleSide, clearcoat: 0.12, clearcoatRoughness: 0.12, envMapIntensity: 1.12,
    }),
